...
#include "MuonEff/MuonEff/src/TMVAClassification_BDT.class.C"
...
class MuonTMVA : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit MuonTMVA(const edm::ParameterSet&);
      ~MuonTMVA();
...
MuonTMVA::MuonTMVA(const edm::ParameterSet& iConfig)
{
  ...
  tr_true->Branch("tmva", &b_tmva, "tmva/F");
  ...
  tr_fake->Branch("tmva", &b_tmva, "tmva/F");

  string dummy[] = { "muonPt", "muonEta", "muonPhi", "isGlobalMuon", "isPFMuon", "chi2LocalPosition", "trkKink", "segmentCompatibility", "normalizedChi2", "numberOfValidMuonHits", "numberOfMatchedStations", "pv0pos_dxy", "pv0pos_dz", "numberOfValidPixelHits", "trackerLayersWithMeasurement" };
...
void MuonTMVA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (auto& mu : *muons) {
    ...
    tmvaValues.push_back(mu.numberOfMatchedStations());
    if ( mu.globalTrack().isNonnull() ){
    ...
    tmvaValues.push_back(mu.pt());
    tmvaValues.push_back(mu.eta());
    tmvaValues.push_back(mu.phi());
    tmvaValues.push_back(b_global);
    tmvaValues.push_back(b_pf);
    tmvaValues.push_back(b_chi2);
    tmvaValues.push_back(b_chi2pos);
    tmvaValues.push_back(b_trkKink);
    tmvaValues.push_back(b_segcompati);
    tmvaValues.push_back(b_nstations);
    tmvaValues.push_back(b_nglobalhits);
    tmvaValues.push_back(b_trackdxy);
    tmvaValues.push_back(b_trackdz);
    tmvaValues.push_back(b_ninnerhits);
    tmvaValues.push_back(b_trackerlayers);
    b_tmva = bdt_->GetMvaValue(tmvaValues);
...
void MuonTMVA::collectGenMuons(const std::vector<reco::GenParticle>& genParticles, std::vector<reco::GenParticle>& genMuons, std::vector<reco::GenParticle>& genMuonsFromZ) const
{
...
void MuonTMVA::fillHitMap(const reco::Muon& mu, TH1D* h_hitmap) const
{
