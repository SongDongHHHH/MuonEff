...
class matchByHit : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit matchByHit(const edm::ParameterSet&);
      ~matchByHit();
...
matchByHit::matchByHit(const edm::ParameterSet& iConfig):
  ...
  //tpToken_ = consumes<TrackingParticleCollection >(iConfig.getParameter<edm::InputTag>("tp"));
...
matchByHit::~matchByHit(){}

void matchByHit::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  h_nevents->Fill(0);
  ...
  assoByHits->associateMuons(recSimColl, simRecColl, muonCollectionH, trackType_, TPCollectionH, &iEvent, &iSetup);
...
void matchByHit::collectGenMuons(const std::vector<reco::GenParticle>& genParticles, std::vector<reco::GenParticle>& genMuons, std::vector<reco::GenParticle>& genMuonsFromZ) const
{
...
bool matchByHit::tightByStep(const reco::Muon& mu, reco::Vertex pv0) const
{
...
void matchByHit::fillHitMap(const reco::Muon& mu, TH1D* h_hitmap) const
{
