...
strMuon = " Muon "
binninglst = {"efficPt":[20,5,105],"effic":[24,0,2.4],"phi":[30,-3,3]}
datadir = os.environ["CMSSW_BASE"]+'/src/MuonEff/MuonEff/test/'

for plotvar in ["effic","efficPt","fakerate","fakerate050","fakeratePt","qOverPtres_vs_eta_sim_Sigma","qOverPtres_vs_pt_sim_Sigma"]:
#for plotvar in ["effic","efficPt","fakerate","fakeratePt","invptres_vs_eta_Sigma","invptres_vs_pt_Sigma"]:
    ...
    hinit = ROOT.TH1F("","",nbins,h_ph2pu0.GetBinLowEdge(1),h_ph2pu0.GetBinLowEdge(nbins+1))
    ...
    y_name = id + strMuon
    if "eff" in plotvar:
        hinit.SetMaximum(1.2)
        hinit.SetMinimum(0.6)
        y_name = y_name+"Efficiency"
    elif "fake" in plotvar:
        hinit.SetMaximum(max(h.GetMaximum() for h in hlist)*1.8)
    ...
    hinit.GetXaxis().SetTitle(x_name)
    hinit.GetYaxis().SetTitle(y_name)
    hinit.GetYaxis().SetTitleOffset(0.98)
    hinit.Draw()

    leg = ROOT.TLegend(0.6,0.72,0.85,0.88)
    setMarkerStyle(h_ph2pu0, 4, 20)
    setMarkerStyle(h_ph2pu140, 1, 34)
    setMarkerStyle(h_ph2pu200, 2, 21)
    for h in hlist:
        h.Draw("e1same")
